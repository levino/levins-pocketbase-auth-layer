 ...
 */
export function generateLoginPageHtml(config) {
    const { pocketbaseUrl, pocketbaseUrlMicrosoft, title = "Login", cssUrl, providers = ["github", "google", "microsoft"], cookieEndpoint = "/api/cookie", redirectUrl = "/", } = config;
 ...
 */
export function generateNotAuthorizedPageHtml(config) {
    const { userEmail, requestAccessEmail, title = "Access Denied", message = "You are logged in but do not have access to this resource.", cssUrl, logoutEndpoint = "/api/logout", } = config;
    const cssLink = cssUrl
        ? `<link rel="stylesheet" href="${cssUrl}">`
        : `<style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
